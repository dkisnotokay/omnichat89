...
- **Счётчик зрителей** — число зрителей обеих платформ без авторизации (в приложении и OBS-оверлее)
- **OBS-оверлей** — Browser Source для стрима (localhost), настройки и модерация синхронизируются в реальном времени
- **Озвучка (TTS)** — два движка: Edge TTS (онлайн, нейроголоса) или **локальные голоса Windows (полностью оффлайн)**; фильтры, очередь со счётчиком пропусков, случайный голос
- **Кастомизация** — любой системный шрифт, насыщенность, обводка/тень текста, анимации сообщений, авто-скрытие по таймеру
- **Twitch OAuth** — бейджи и эмоуты через Helix API
- **Системные события** — подписки, рейды, подарки, выделенные сообщения
- **Модерация** — удаление сообщений, баны, очистка чата по платформам (приложение + оверлей)
...
| Озвучка | Microsoft Edge TTS (WebSocket) / Windows SpeechSynthesis (WinRT, оффлайн) |
