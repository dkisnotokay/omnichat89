...
/// Ссылка на эмоут или GIF в тексте сообщения.
/// Указывает позиции начала и конца в тексте, где находится картинка.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EmoteRef {
    ...
    pub end: usize,
    /// GIF (Twitch, тег `gifs`) — рендерится крупнее эмоута.
    /// `default` для совместимости со старыми сохранёнными данными.
    #[serde(default)]
    pub is_gif: bool,
}
